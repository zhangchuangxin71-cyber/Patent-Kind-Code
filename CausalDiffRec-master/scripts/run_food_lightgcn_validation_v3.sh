...
MASTER_LOG=logs/food_lightgcn_validation_v3_master.log
VALIDATION_REPORT=experiments/reports/food_lightgcn_late_fusion_validation_v3.json
...
status "Food five-seed parallel LightGCN start: free=${free_mib} MiB"
pids=()
for seed in "${SEEDS[@]}"; do
    record="experiments/records/food_lightgcn_strict_ood_seed${seed}_v3.json"
    checkpoint="checkpoints/food_lightgcn_strict_ood_seed${seed}_v3.pt"
    log="logs/food_lightgcn_v3_seed${seed}.log"
    if [[ -f "$record" ]]; then
...
env PYTHONUNBUFFERED=1 CUDA_VISIBLE_DEVICES=0 "$PYTHON_BIN" \
    ...
    --source_records \
        experiments/records/food_lightgcn_strict_ood_seed1024_v3.json \
        experiments/records/food_lightgcn_strict_ood_seed2048_v3.json \
        experiments/records/food_lightgcn_strict_ood_seed3072_v3.json \
        experiments/records/food_lightgcn_strict_ood_seed4096_v3.json \
        experiments/records/food_lightgcn_strict_ood_seed5120_v3.json \
...
for seed in "${SEEDS[@]}"; do
    source="experiments/records/food_lightgcn_strict_ood_seed${seed}_v3.json"
    out="experiments/records/food_lightgcn_latefusion_strict_ood_seed${seed}_v3.json"
    log="logs/food_lightgcn_latefusion_test_v3_seed${seed}.log"
    if [[ -f "$out" ]]; then
...
"$PYTHON_BIN" scripts/summarize_late_fusion_test.py \
    --records experiments/records/food_lightgcn_latefusion_strict_ood_seed*_v3.json \
    --validation_report "$VALIDATION_REPORT" \
    --out experiments/reports/food_lightgcn_late_fusion_test_v3.json \
    > logs/food_lightgcn_late_fusion_test_summary_v3.log 2>&1
status "FOOD_LIGHTGCN_LATE_FUSION_PIPELINE_V3_COMPLETE"
