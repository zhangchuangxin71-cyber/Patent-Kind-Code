...
def compute_beta(epoch, total_epochs, start=1.0, end=1.0):
    """Linearly interpolate the risk-mean weight over training.

    ``start=end=1`` preserves the corrected-v3 objective.  Exposing both ends
    makes an annealing experiment explicit instead of hiding a tautology in
    the implementation.
    """
    if total_epochs <= 0:
...
def fast_evaluation(epoch, measure, bestPerformance, select_metric="NDCG"):
    ...
    if len(bestPerformance) > 0:
        ...
        if select_metric not in performance:
            raise KeyError(f"Selection metric {select_metric!r} missing from {performance}")
        if performance[select_metric] > bestPerformance[1][select_metric]:
...
def mask_seen_items(scores, interaction_mat, user_set=None):
    """Return scores with train-observed user-item pairs set to ``-inf``.

    ``scores`` is [n_user, n_item] and ``interaction_mat`` is a SciPy CSR with
    the same logical shape.  Restricting to ``user_set`` avoids touching rows
    ...
    if user_set is None:
        ...
        users = np.asarray(sorted(int(u) for u in user_set), dtype=np.int64)
        coo = interaction_mat[users].tocoo()
        rows = users[coo.row]
    if coo.nnz:
        row_idx = torch.as_tensor(rows, dtype=torch.long, device=masked.device)
        col_idx = torch.as_tensor(coo.col, dtype=torch.long, device=masked.device)
        masked[row_idx, col_idx] = -torch.inf
    return masked


def get_rec_list(user_set, scores, max_user_id, topk=None):
    rec_list = {}
    user_list = sorted(int(u) for u in user_set)

    user_indices_tensor = torch.tensor(user_list, dtype=torch.long, device=scores.device)

    candidate_count = scores.shape[1]
    k = candidate_count if topk is None else min(int(topk), candidate_count)
    sorted_user_scores, sorted_indices = torch.topk(scores[user_indices_tensor], k=k, dim=1)
    ...
    for idx, user_index in enumerate(user_indices_tensor):
        ...
        formatted_scores = list(zip(map(str, sorted_items.tolist()), sorted_user_scores[idx].tolist()))
...
def normalize_graph_mat(adj_mat):
    ...
    if shape[0] == shape[1]:
        ...
        rowsum = np.maximum(rowsum, 1e-10)  # 确保所有值 >= 1e-10#新增
