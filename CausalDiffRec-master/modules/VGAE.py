...
class Model(nn.Module):
    ...
    def encoder(self, features, edge_index, edge_weight=None, sample=True):
        """Encode a graph, optionally sampling from the variational posterior."""
        h = self.base_gc(features, edge_index, edge_weight=edge_weight).to(self.device)
        self.mean = self.mean_gc(h, edge_index, edge_weight=edge_weight)
        self.log_std = self.log_std_gc(h, edge_index, edge_weight=edge_weight).clamp(min=-10.0, max=5.0)
    ...
    def forward(self, features, edge_index, edge_weight=None):
        z = self.encoder(features, edge_index, edge_weight=edge_weight)
