...
class LGCN_Encoder(nn.Module):
    def __init__(self, user_num, n_layers, norm_adj, user_embeddings=None, item_embeddings=None):
        ...
        self.sparse_norm_adj = self.convert_sparse_mat_to_tensor(self.norm_adj)
    ...
    def forward(self):
        ...
        for k in range(self.layers):
            ego_embeddings = torch.sparse.mm(self.sparse_norm_adj.to(ego_embeddings.device), ego_embeddings)
            all_embeddings += [ego_embeddings]
